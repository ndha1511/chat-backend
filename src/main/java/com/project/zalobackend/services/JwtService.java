...
public class JwtService {
    ...
    public boolean isTokenValid(String token, UserDetails userDetails) {
        ...
        Optional<Token> optionalRefreshToken = tokenRepository.findByRefreshToken(token);
        if(optionalToken.isEmpty()) {
            if(optionalRefreshToken.isEmpty()) return false;
            else
                if(!optionalRefreshToken.get().isRevoked()) return false;
        } else {
